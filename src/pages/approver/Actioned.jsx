...
const Actioned = () => {
  ...
  const getFileIcon = (type = "") => {
    ...
    if (ext.includes("word") || ext.includes("doc"))
      return <FileWordOutlined style={{ color: PRIMARY_BLUE }} />;
    if (ext.includes("xls") || ext.includes("excel"))
  ...
  const modalCustomStyles = `
    ...
    .actioned-modal-container {
      background: white;
      border-radius: 12px;
      overflow: visible;
      width: 1200px;
      max-width: calc(100vw - 310px);
      box-shadow: 0 10px 30px rgba(22, 70, 121, 0.12);
  ...
  const selectedNextDueDateValue =
  ...
  const renderDclUpload = () => {
    return (
      <Card
        size="small"
        title={`Mandatory: DCL Upload ${dclDocs.length > 0 ? "✓" : ""}`}
        style={{ marginTop: 12 }}
      >
        {dclDocs.length === 0 ? (
          <div style={{ textAlign: "center", color: WARNING_ORANGE }}>
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#666" }}>
                      {doc.size ? `${(doc.size / 1024).toFixed(2)} MB` : ""}{" "}
                      {doc.uploadDate
                        ? `• Uploaded: ${dayjs(doc.uploadDate).format("DD MMM YYYY HH:mm")}`
                        : ""}
                    </div>
                  </div>
                  <div
            ...
            return (
              <List.Item key={idx} style={{ padding: "6px 0" }}>
                ...
                >
                  ...
                  >
                    {timestamp ? dayjs(timestamp).format("M/D, h:mm A") : ""}
                  </div>
                </div>
              </List.Item>
            );
  ...
  const downloadDeferralAsPDF = async () => {
    ...
    try {
      ...
      doc.setFont(undefined, "bold");
      doc.text(
        `Deferral Request: ${selected.deferralNumber || "N/A"}`,
        margin,
        10,
      );
      yPosition = 25;
      ...
      const deferralDetailsItems = [
        ...
        {
          label: "DCL No",
          value: selected.dclNo || selected.dclNumber || "N/A",
        },
        { label: "Status", value: selected.status || "Pending" },
        {
          label: "Creator Status",
          value: selected.creatorApprovalStatus || "Pending",
        },
        {
          label: "Creator Date",
          value: selected.creatorApprovalDate
            ? dayjs(selected.creatorApprovalDate).format("DD/MM/YY")
            : "N/A",
        },
        {
          label: "Checker Status",
          value: selected.checkerApprovalStatus || "Pending",
        },
        {
          label: "Checker Date",
          value: selected.checkerApprovalDate
            ? dayjs(selected.checkerApprovalDate).format("DD/MM/YY")
            : "N/A",
        },
        {
        ...
        {
          label: "Created At",
          value: dayjs(selected.createdAt).format("DD MMM YYYY HH:mm"),
        },
      ];
      ...
      if (selected.deferralDescription || selected.description) {
        const descText =
          selected.deferralDescription || selected.description || "";
      ...
      const approvalFlow =
        Array.isArray(selected.approverFlow) && selected.approverFlow.length > 0
          ? selected.approverFlow
          : Array.isArray(selected.approvers)
            ? selected.approvers
            : [];
      ...
      doc.save(
        `Deferral_${selected.deferralNumber}_${dayjs().format("YYYYMMDD")}.pdf`,
      );
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        ...
        <Tabs.TabPane
          tab={`Extension Applications (${actionedExtensions.length})`}
          key="extensions"
        />
      </Tabs>
            ...
            <Row gutter={[16, 16]} align="middle">
              <Col xs={24} sm={12} md={8}>
                <Input
                  ...
                  onChange={(e) => {
                    /* Implement search filter if desired */
                  }}
                  allowClear
                  size="middle"
                />
              ...
              <Empty
                description={
                  <div>
                    ...
                    <p style={{ color: "#999" }}>
                      All actioned items are shown here
                    </p>
                  </div>
                }
                style={{ padding: 40 }}
      ...
      {/* Custom Modal Overlay with Blue Header */}
      <div
        className="actioned-modal-overlay"
        style={{
      ...
      >
        {modalOpen && selected && (
          <div
            className="actioned-modal-container"
            ...
            >
                              ...
                              uploadedVersion.url && (
                                <>
                                  <Button
                                    ...
                                    icon={<EyeOutlined />}
                                    onClick={() =>
                                      handleViewDocument(uploadedVersion)
                                    }
                                    size="small"
                                  >
                                    View
                                  ...
                                  <Button
                                    ...
                                    icon={<DownloadOutlined />}
                                    onClick={() =>
                                      handleDownloadDocument(uploadedVersion)
                                    }
                                    size="small"
                                  >
                                    Download
              ...
              {selected.facilities && selected.facilities.length > 0 && (
                <Card
                  ...
                  title={
                    <span style={{ color: PRIMARY_BLUE }}>
                      Facility Details ({selected.facilities.length})
                    </span>
                  }
                  style={{ marginBottom: 18, marginTop: 12 }}
                >
                  <Table
                    dataSource={selected.facilities}
              ...
              <Card
                ...
                style={{
                  marginBottom: 18,
                  opacity: selected?.status === "rejected" ? 0.6 : 1,
                }}
              >
                ...
                <div
                  style={{
                    ...
                    gap: 12,
                    pointerEvents:
                      selected?.status === "rejected" ? "none" : "auto",
                  }}
                >
                  {(function () {
                    const approvers = [];
                    ...
                    if (
                      selected?.approverFlow &&
                      Array.isArray(selected.approverFlow)
                    ) {
                      hasApprovers = true;
              ...
              <div style={{ marginTop: 24, marginBottom: 24 }}>
                ...
                <CommentTrail history={selectedHistory} isLoading={false} />
              </div>
